...
LAOL 内置了**实时上下文注入流水线**，在每个智能体任务执行前后运行真实的工具链。这避免了智能体自己执行 `tsc`、`eslint` 或 `git blame` 浪费 token——诊断结果已被预计算。
...
上下文注入系统是语言感知的。完整覆盖因语言而异：

| 语言 | 类型检查 | Lint | 测试 | 符号索引 | Git | 自定义 | 覆盖率 |
|------|---------|------|------|---------|-----|--------|--------|
| **TypeScript/JS** | tsc | eslint | vitest/jest | TS AST | 是 | 是 | 7/7 个 Provider |
| **Python** | mypy | ruff | pytest | Python AST | 是 | 是 | 5/7 个 Provider |
| **其他**（Go、Rust、Java 等） | — | — | — | — | 是 | 是 | 2/7 个 Provider |

**TypeScript/JavaScript** 项目获得完整流水线：类型检查（tsc）、lint（eslint）、测试运行（vitest/jest）、符号索引（TS 编译器 API）以及 Git 诊断——所有功能均支持 tree-hash 缓存，避免多智能体间的重复运行。

**Python** 项目拥有扎实的诊断覆盖（ruff + mypy，均支持 tree-hash 缓存）和测试支持（pytest 测试文件推断），但没有 eslint 的等价物。Ruff 已覆盖大多数 lint 规则，此差距在实践中较窄。符号索引支持 Python AST 提取，包括 Sphinx/Google 风格 docstring 解析。

**其他语言** 仅可使用 `GitProvider`（任何 git 仓库均可用）和 `CustomProvider`（用户自定义前置/后置命令）。如需为其他语言添加诊断，在 `context_providers` 中配置 `custom`，提供以 grep 友好格式输出错误的 shell 命令。

**智能提示路由：** 关键诊断（类型错误、lint 违规、测试结果）直接注入智能体 prompt。低优先级上下文（git blame、代码符号）写入 `.multiagent/diagnostics.md`——按需读取，不消耗 prompt token。每任务节省约 350 token。

**跨 Provider 去重：** 当 TypeScript 和 ESLint 报告同一代码问题时，自动检测并过滤重复项（>60% 内容重叠检测），避免冗余上下文浪费 token。
...
## ChangeJournal 变更日志（拉取式变更追踪）

LAOL 采用**拉取式 ChangeJournal**，而非向所有智能体广播每次文件变更（会对处理无关文件的智能体产生噪音）。智能体按需查询与其目标文件相关的变更。

| 组件 | 用途 |
|------|------|
| **ChangeJournal**（调度器） | 记录文件变更、索引更新和合并完成事件到 `.multiagent/journal/change-log.ndjson`（追加式 NDJSON 日志）。内存上限 10,000 条。 |
| **ChangeJournalClient**（智能体） | 任务启动时通过 `change_query` socket 消息查询日志，过滤与目标文件匹配的条目，写入本地缓存 `.multiagent/journal/latest-changes.md`。 |
| **定向知识推送** | 知识更新仅推送给忙碌中的智能体（正在执行任务的），而非广播给所有已连接智能体。智能体将收到的知识写入 `.multiagent/notifications.md`。 |

**效果：** 智能体只看到与自己相关的变更。文件变更噪音被消除；知识共享得以保留。
...
这避免了智能体重复探索相同代码或重复已知错误。存储直接使用文件系统——每个任务一个 JSON 文件——支持 O(1) 按任务 ID 查找。内存缓存（5 分钟 TTL）通过就地更新避免每次查询都重新读取文件。
...
- **智能体任务定位** — 执行任务前，`CodebaseProvider` 查询索引获取目标文件中的符号，将 `[CODEBASE SYMBOLS]` 上下文写入 `.multiagent/diagnostics.md`。这帮助 LLM 无需遍历全部文件即可理解代码库结构。
...
| 优化项 | 说明 |
|--------|------|
| **拉取式 ChangeJournal** | 智能体按需查询目标文件的变更，而非接收每次文件修改的广播。消除了无关文件变更对智能体造成的噪音。 |
| **提示优先级路由** | 关键诊断（tsc、eslint、test）直接注入 prompt。Git 和 codebase 上下文写入 `.multiagent/diagnostics.md` 供按需读取，每任务节省约 350 token。 |
| **跨 Provider 去重** | 通过内容重叠分析（>60% 阈值），检测并过滤来自不同 Provider 的重复诊断（如同一错误被 tsc 和 eslint 同时报告）。 |
...
| **定向知识推送** | 知识更新仅发送给忙碌中的智能体（有活跃任务的），而非广播给所有已连接智能体。 |
| **TaskWatcher 去重** | 追踪已发送的任务 ID，避免 chokidar 重复触发或轮询重叠导致的冗余冲突检查。 |
| **KnowledgeStore 就地缓存** | 保存时直接更新缓存条目，而非使缓存失效，避免重新读取全部文件。 |
...
| **上下文注入流水线** | 每个任务执行前，7 个实时 Provider（tsc、eslint、test、git 等）运行诊断。关键结果直接注入 prompt；完整结果写入 `.multiagent/diagnostics.md`。任务完成后，Provider 计算变更差异，使下一智能体了解变化。 |
| **智能体熔断器** | 2 次连续失败 → 降级（仅接受简单任务）。5 次 → 隔离（不再接受任务）。防止故障智能体持续消耗 API 费用。 |
| **任务依赖链** | 当任务 B 依赖任务 A 时，B 的 Worktree 从 A 的分支创建——继承全部代码变更。智能体收到 `[PREDECESSOR]` 上下文提示了解 A 做了什么。任意长度的链 A → B → C 自然工作。 |
...
- **拉取式变更追踪** — 智能体从 ChangeJournal 查询与其目标文件相关的变更。文件变更不推送；知识更新仅推送给忙碌中的智能体。
...
- **实时工具链查询，而非静态解析** — Context Provider 运行真实的编译器、linter 和测试运行器，而非仅依赖 AST 索引。
...
```jsonc
{
  ...
  "agent": {
    ...
    "worktree_pool_size": 1,   // 每智能体的 worktree 数量（默认 1）
...
任务执行期间，智能体可在其 Worktree 的 `.multiagent/` 目录中访问以下文件：

| 文件 | 用途 |
|------|------|
| `.multiagent/diagnostics.md` | 所有 Context Provider 的预检诊断（tsc、eslint、test、git、codebase）。任务开始前写入。 |
| `.multiagent/journal/latest-changes.md` | 与智能体目标文件相关的最近文件变更、索引更新和合并信息（从 ChangeJournal 查询获得）。 |
| `.multiagent/notifications.md` | 其他智能体分享的知识——来自已完成任务的学习、摘要和洞察。 |
...
AgentRunner.handleTaskAssigned(消息)
    ...
    └── AgentWorker.executeTask(任务, 执行器)
        ...
        ├── ChangeJournalClient.query()    # 拉取目标文件的最近变更
        │   └── 写入 .multiagent/journal/latest-changes.md
        ├── ContextManager.collectPreHints()   # 7 个实时 Provider 并行运行
        │   ├── 若 tree hash 匹配，从缓存读取 TSC 结果（跨智能体共享）
        │   ├── 关键提示（tsc、eslint、test）→ 直接注入 prompt
        │   └── 完整诊断 → .multiagent/diagnostics.md
        ├── Checkpoint.checkAndRebase()    # 拉取最新 main，必要时 rebase
        │
        ├── KnowledgeStore.getByTaskId()   # 查询前序任务知识（O(1)）
        ...
        ├── KnowledgeStore.saveDelta()     # 保存 Provider 差异
...
npm test            # Vitest（287 个测试，21 个文件）
npm run dev         # 监视模式
```
...
├── journal/         # ChangeJournal + 智能体查询客户端（拉取式变更追踪）
...
├── codebase/        # 符号级索引器（TS/Python AST 提取、关键词搜索、API 文档生成）
├── cli/             # 基于 Commander 的 CLI
└── __tests__/       # 21 个测试文件，287 个测试
```
